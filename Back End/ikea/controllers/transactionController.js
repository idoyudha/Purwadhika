...
module.exports = {
    ...
    moveToTransaction: async (request, response, next) => {
        try {
            let queryTransaction = `INSERT INTO db_ikea.transaction (invoice, date, iduser, delivery_cost, total_payment, note, idstatus) 
            VALUES ('AA1', NOW(), ${request.params.iduser}, 40000, (SELECT SUM(quantity * price) total_payment 
            FROM db_ikea.cart JOIN db_ikea.product ON cart.idproduct = product.idproduct WHERE iduser = 3), 'NOTE', 6);`
